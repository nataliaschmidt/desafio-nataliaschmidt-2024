...
describe('Recintos do Zoologico', () => {
    ...
    test('Deve encontrar recintos para 1 macaco em que ele não fique sem outros animais', () => {
    ...
    test('Animais carnívoros somente habitam com sua própria espécie', () => {
        ...
        expect(resultado.recintosViaveis).not.toContain('Recinto 1 (espaço livre: 4 total: 10)');
        expect(resultado.recintosViaveis.length).toBe(1);
    });

    test('Quando há mais de uma espécie no mesmo recinto, é preciso considerar 1 espaço extra ocupado', () => {

        const resultado = new RecintosZoo().analisaRecintos('GAZELA', 1);
        expect(resultado.erro).toBeFalsy();
        expect(resultado.recintosViaveis[0]).not.toContain('Recinto 1 (espaço livre: 5 total: 10)');
    ...
    test('Hipopótamo(s) só tolera(m) outras espécies estando num recinto com savana e rio', () => {
        ...
        expect(resultado.erro).toBeFalsy();
        expect(resultado.recintosViaveis[0]).not.toContain('Recinto 1 (espaço livre: 2 total: 10)');
